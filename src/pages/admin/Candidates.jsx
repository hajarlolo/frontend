...
import { fetchAdminCandidates, adminDecisionVerification, fetchAccountDocument } from "../../services/endpoints";
...
export default function Candidates() {
  ...
  const moderateMutation = useMutation({
    ...
    onSuccess: () => queryClient.invalidateQueries(["admin-candidates"]),
  });
  ...
  return (
    <AppShell title="Gestion des Candidats" subtitle="Étudiants et Lauréats inscrits">
      <Card>
        <form onSubmit={handleSearch} className="flex flex-wrap gap-4 items-end">
          <Input name="search" label="Recherche" placeholder="Nom, email..." className="flex-1" />
          <Select name="type" label="Type" className="w-48">
            ...
            <tbody className="divide-y divide-slate-200">
              {isLoading ? (
                ...
                candidates.map((candidate) => {
                  const status = candidate.user?.latest_verification?.status || "Inconnu";
                  const verificationId = candidate.user?.latest_verification?.id_verification;
                  
                  return (
                    <tr key={candidate.id_candidat} className="hover:bg-slate-50 transition-colors">
                      <td className="px-6 py-4 font-medium text-slate-900">
                        {candidate.user?.nom}
                      </td>
                      <td className="px-6 py-4">
                        <span className={`px-2 py-1 rounded inline-flex text-xs font-semibold ${candidate.user?.role === 'student' ? 'bg-blue-50 text-blue-700' : 'bg-purple-50 text-purple-700'}`}>
                          {candidate.user?.role === 'student' ? 'Étudiant' : 'Lauréat'}
                        </span>
                      </td>
                      <td className="px-6 py-4">
                        <div className="text-slate-500 text-xs">{candidate.user?.email}</div>
                      </td>
                      <td className="px-6 py-4">
                        <Badge value={status} />
                      </td>
                      <td className="px-6 py-4 text-center">
                        <Button size="sm" variant="ghost" onClick={() => openDoc(candidate.user?.id_user)}>
      ...
      <Modal 
        ...
        title="Justificatif de vérification"
        maxWidth="max-w-4xl"
      >
